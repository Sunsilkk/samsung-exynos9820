...
then
	...
	if [ "x$1" = "x" ]; then
		nver="$(curl -sL https://github.com/HuskyDG/magisk-files/raw/main/stable.json | jq -r '.magisk.version')"
	else
		nver="$1"
	fi
	magisk_link="https://huskydg.github.io/download/magisk/$nver.apk"
fi
